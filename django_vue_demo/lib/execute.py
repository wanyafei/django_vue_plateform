...
from apps.apitest.models import Case,Interface
import logging
#案例执行类方法
class Execute():
    ...
    def run_case(self):
        ...
        case_content=eval(case_obj.content)  #case_content
    ...
    def step(self,content):
        ...
        #接口请求
        if_info_dict = {"url": inter_obj.if_url, "header": self.transform_parm(content["header"]), "body": self.transform_parm(content["body"])}
    ...
    def transform_parm(self,list):
        ...
        res = {}
        for item in list:
            res[item['var_name']]=item['var_remark']
        return res
    def set_extract(self,extract_dict,res_counter):
        '''
        将提取值存放到对应的全局字典项中
        :param extract_dict: 提取字段列表
        :param res_counter: 接口返回的响应信息
        :return:
        '''
        for k in extract_dict:
            extract_val=k['var_name']
            k_value=self.get_parms(extract_val,res_counter)  #get_parms()函数，在返回内容counter中拿到提取变量对应的value值
            self.extract_global_dict[extract_val]=k_value
...
if __name__ == '__main__':
    data=[{'var_name':'content_type','var_remark':'application/json;charet=UTF-8'},{'var_name':'aaa','var_remark':'111'}]
    print(Execute(11, 22).transform_parm(data))